...
func NewsReviewSelectList(pageSize int, pageNum int, search NewsReviewSearchVo) (news_reviews []NewsReview, err error) {
	whereStr, args := GenWhereByStruct(search)
	sql, _ := ReplaceQuestionToDollarInherit("select id,new_id,content,create_time,ip,user_id,parent_id,email,user_name,url,img,state from news_review"+whereStr+" order by state limit ? offset ?", 0)
	fmt.Println(sql)
